...
import logoimage from "../pages/MainPage/MainImgs/logo.png";
...
const LoginAgree = () => {
  ...
  const submitAgree = async () => {
    ...
    axios
      .post(
        `${process.env.REACT_APP_AMUSE_API}/api/v1/user/login/info`,
        requestBody,
        {
          headers: {
            "Content-Type": "application/json",
      ...
      >
        ...
        >
          ...
          >
            <input
              type="checkbox"
              style={{ margin: "0px", marginLeft: 24 }}
              value={marketingAgree}
              onChange={() => {
                setPrivacy(!privacy);
              }}
            />
            <div style={{ marginLeft: 24, width: "calc(100% - 100px)" }}>
              {"개인정보 활용동의 (필수)"}
            </div>
